...
/*
     ...
     if (preg_match("/^(\d{1,2})(\/|-)(\d{1,2})\2(\d{4})$/",$DOB)) {
       $age = age($_POST["DOB"]);   
       $ageErr = ""; 
     }
     $DOB = $_POST["DOB"];
...
try {
    ...
    $sql = "INSERT INTO Client (LastName, FirstName, MI, FullName, Suffix, SSN) 
    SELECT * FROM ('$LastName', '$FirstName', '$MI', '$FullName', '$Suffix', '$SSN') AS tmp
    where not exist ( SELECT FullName FROM Client WHERE FullName = '$FullNamecheck') LIMIT 1";
...
<div class="form-group">
   ...
   <input type="date" class="form-control" id="bday" placeholder="mm/dd/yyyy" onkeypress="return isDateKey(event)" onkeyup="myAgeValidation()" onkeydown="return addSlashes(this)" name="DOB" maxlength="10" required value="<?php echo $DOB = date_format($DOB,'m/d/Y');?>" autofocus>
   <br>
